...
$ipnyo = $_SERVER['HTTP_X_FORWARDED_FOR']." Proxy: ".$_SERVER['REMOTE_ADDR'];
}else{
...
function loopmenucp($id,$level) {
        ...
        $rssub = $koneksi_db->prepare("SELECT `id`,`ordering`,`tipe`, `judul_id` judul, `whoentri`, `whereentri`, `whenentri`, `whoubah`, `whereubah`, `whenubah`, `published` FROM `".$namadepan."menu` WHERE `subid` = ? ORDER BY `ordering`");
        $rssub->execute(array($id));
                $rssub->setFetchMode(PDO::FETCH_ASSOC);
        
        if ($rssub->rowCount() > 0) {
        echo "\n";
        while ($rwsub = $rssub->fetch()) {
        $orderup = $rwsub['ordering'] - 1;
        $orderdown = $rwsub['ordering'] + 1;
        $targetup = $koneksi_db->prepare("select `id` from `".$namadepan."menu` where `ordering`=$orderup AND `subid`=?");
        $targetup->execute(array($id));
        $targetup->setFetchMode(PDO::FETCH_ASSOC);
        $rtargetup = $targetup->fetch();
        
        
        $targetdown = $koneksi_db->prepare("select `id` from `".$namadepan."menu` where `ordering`=$orderdown AND `subid`=?");
        $targetdown->execute(array($id));
        ...
        echo "<a href=\"?cpmodul=$namamodul&aksi=ngurutke&idt=".$rtargetup['id']."&id=".$rwsub['id']."&order=".$rwsub['ordering']."&ordert=$orderup\"><i class=\"icon-arrow-up btn btn-blue\"></i></a>\n";
        if (isset($rtargetdown['id']))
        echo "<a href=\"?cpmodul=$namamodul&aksi=ngurutke&idt=".$rtargetdown['id']."&id=".$rwsub['id']."&order=".$rwsub['ordering']."&ordert=$orderdown\"><i class=\"icon-arrow-down btn btn-blue\"></i></a>\n";
        
        
        if ($modulubah->rowCount() > 0)
        {
        if ($rwsub['published'] == 'y')
        $publish = "<a href=\"?cpmodul=$namamodul&aksi=published&id=".$rwsub['id']."\"><i class=\"icon-ok btn btn-green\"></i></a>";
        if ($rwsub['published'] == 't')
        $publish = "<a href=\"?cpmodul=$namamodul&aksi=published&id=".$rwsub['id']."\"><i class=\"icon-remove btn btn-red\"></i></a>";
        $ubah = "<a href=\"?cpmodul=$namamodul&aksi=ubah&id=".$rwsub['id']."\"><i class=\"icon-pencil btn btn-grey\"></i></a>";
        ...
        $rssub2 = $koneksi_db->prepare("SELECT `id`,`judul_id` judul FROM `".$namadepan."menu` WHERE `id` = ? ORDER BY `ordering`");
        $rssub2->execute(array($id));
                $rssub2->setFetchMode(PDO::FETCH_ASSOC);
        
        if ($rssub->rowCount() > 0) {
        while ($rwsub2 = $rssub2->fetch()) {
...
function recursive_table_subunitkerja($id,$level = 1,$pengguna = NULL) {
    ...
    $suk->setFetchMode(PDO::FETCH_ASSOC);
           ...
           if($pengguna == 1)
           {
                       $sub .= '<tr class="treegrid-'.$rwsub['id'].'  treegrid-parent-'.$rwsub['induk'].'"><td>'.$rwsub['kode'].' - '.$rwsub['nama'].'</td><td><a href="?cpmodul=pengguna&aksi=daftarpengguna&id='.$rwsub['id'].'&suk='.$_GET['id'].'" class="btn" rel="tooltip" title="Daftar Pengguna"><i class="icon-group"></i></a> 
                       </td></tr>';
           }
           else
           {
