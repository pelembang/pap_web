...
$modulhapus->setFetchMode(PDO::FETCH_ASSOC);
	...
	$aColumns = array("'aksi' aksi","h.id","concat('<p><b>ID |</b> ',judul_id,'</p><p><b>EN |</b> ',judul_en,'</p>') judul","p.nama pengirim","concat_ws('<br/>',if(h.published=1,'Dipublikasi','Konsep'),date_format(h.whenentri, '%d %b %Y %H:%i')) waktu","h.hits" );	
	$bColumns = array("aksi","id","judul","pengirim","waktu","hits" ); //columns for order
	$cColumns = array("h.judul_id","h.judul_en","p.nama" ); //columns for where

	$sIndexColumn = "h.id";
	
	$sTable = $namadepan."halaman h";
	$sJoin = "left join ".$namadepan."syspemakai p on p.id = h.whoentri";
	/* 
	 * Paging
	 */
	$sLimit = "";
