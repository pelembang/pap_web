...
$modulhapus->setFetchMode(PDO::FETCH_ASSOC);
	...
	$aColumns = array("'aksi' aksi","a.id","concat('<p><b>ID |</b> ',judul_id,'</p><p><b>EN |</b> ',judul_en,'</p>') judul","concat_ws(' s.d ',date_format(a.wak_awal, '%d %b %Y %H:%i'),if(sd_selesai=1,'Selesai',date_format(a.wak_akhir, '%d %b %Y %H:%i'))) waktu","concat_ws('<br/>',if(a.published='y','Dipublikasi','Konsep'),date_format(a.whenentri, '%d %b %Y %H:%i'),concat('Oleh : ',p.nama)) info" );	
	$bColumns = array("aksi","id","judul","waktu","info" ); //columns for order
	$cColumns = array("a.judul_id","a.judul_en","a.nama" ); //columns for where

	$sIndexColumn = "a.id";
	
	$sTable = $namadepan."agenda a";
	$sJoin = "left join ".$namadepan."syspemakai p on p.id = a.whoentri";
	/* 
	 * Paging
	 */
	$sLimit = "";
