...
$modulhapus->setFetchMode(PDO::FETCH_ASSOC);
	...
	$aColumns = array("'aksi' aksi","p.id","p.singkatan","p.nama_id","p.nama_en","p.strata","p.akreditasi","concat(p.lama,' Tahun') lama" );	
	$bColumns = array("aksi","id","singkatan","nama_id","nama_en","strata","akreditasi","lama" ); //columns for order
	$cColumns = array("p.singkatan","p.nama_id","p.nama_en"); //columns for where

	$sIndexColumn = "p.id";
	
	$sTable = $namadepan."prodi p";
	$sJoin = "";
	/* 
	 * Paging
	 */
