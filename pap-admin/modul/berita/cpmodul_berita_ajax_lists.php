...
$modulhapus->setFetchMode(PDO::FETCH_ASSOC);
	...
	$aColumns = array("'aksi' aksi","b.id","concat('<p><b>ID |</b> ',judul_id,'</p><p><b>EN |</b> ',judul_en,'</p>') judul","p.nama pengirim","k.kat_id kategori","concat_ws('<br/>',if(b.published=1,'Dipublikasi','Konsep'),date_format(b.whenentri, '%d %b %Y %H:%i')) waktu","b.hits" );	
	$bColumns = array("aksi","id","judul","pengirim","kategori","waktu","hits" ); //columns for order
	$cColumns = array("b.judul_id","b.judul_en","p.nama","k.kat" ); //columns for where

	$sIndexColumn = "b.id";
	
	$sTable = $namadepan."berita b";
	$sJoin = "left join ".$namadepan."kategori k on k.id = b.kat left join ".$namadepan."syspemakai p on p.id = b.whoentri";
	/* 
	 * Paging
	 */
	$sLimit = "";
