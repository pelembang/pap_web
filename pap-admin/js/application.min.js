...
var $_GET = getQueryParams(document.location.search);
	...
	$(function(){
		...
		var status=$('#status');
		if ($_GET["cpmodul"] === "notices")
		{
		new AjaxUpload(btnUpload, {
			action: '?ajax=notices&aksi=uploadfile',
			name: 'uploadfile',
			accept: '.rar,.jpg,.jpeg,.doc,.docx,.pdf,.zip',
			onSubmit: function(file, ext){
				 if (! (ext && /^(jpg|jpeg|rar|doc|docx|pdf|zip)$/.test(ext))){ 
                    // extension is not allowed 
					status.text('Only JPG, RAR or DOC files are allowed');
					return false;
				}
				status.html('<img src="./img/plugins/ajaxupload/uploading.gif"> Uploading...');
			},
			onComplete: function(file, response){
				...
				if(stats[0]==="success"){
				var ress = file.split(".");
					var namofiles = stats[1]+ress[0]+ress[1];
					$('<tr filename="'+file+'"></tr>').appendTo('#filelist').html('<td><img border="0" src="./img/plugins/ajaxupload/circle.png"> </td><td>'+file+'<input type="hidden" name="flupload[]" value="'+stats[1]+file+'"></td><td> <a href="#uploafile" id="'+stats[1]+file+'" class="deleteimage"><img border="0" src="./img/plugins/ajaxupload/trash.png"></a></td>').addClass(namofiles).attr("id","success");
					
				} else{
					status.html('Error Upload');
				}
			}
			}
		});
		}
		else if ($_GET["cpmodul"] === "documents")
		{
		new AjaxUpload(btnUpload, {
			action: '?ajax=documents&aksi=uploadfile',
			name: 'uploadfile',
			//accept: '.rar,.jpg,.jpeg,.doc,.docx,.pdf,.zip',
			onSubmit: function(file, ext){
				// if (! (ext && /^(jpg|jpeg|rar|doc|docx|pdf|zip)$/.test(ext))){ 
		...
		else if ($_GET["cpmodul"] === "pengumuman")
		{
		new AjaxUpload(btnUpload, {
			action: '?ajax='+$_GET["cpmodul"]+'&aksi=uploadfile',
			name: 'uploadfile',
			accept: '.jpg,.jpeg,.png,.pdf',
			onSubmit: function(file, ext){
				 if (! (ext && /^(jpg|jpeg|png|pdf)$/.test(ext))){ 
                    // extension is not allowed 
					status.text('Only JPG, PNG or PDF files are allowed');
					return false;
				}
				status.html('<img src="./img/plugins/ajaxupload/uploading.gif"> Uploading...');
			},
			onComplete: function(file, response){
				...
				if (stats[2] !== "image/png" && stats[2] !== "image/jpeg" && stats[2] !== "application/pdf")
				{ 
                    // extension is not allowed 
					status.text('Only JPG, PNG or PDF files are allowed');
					return false;
				...
				if(stats[0]==="success"){
				var ress = file.split(".");
					var namofiles = stats[1]+ress[0]+ress[5];
					var idunixtime = Date.now();
					$('<tr filename="'+file+'"></tr>').appendTo('#filelist').html('<td valign="top"><img border="0" src="./img/plugins/ajaxupload/circle.png"> </td><td valign="top"><a href=\"tmp/'+$_GET["cpmodul"]+'/'+stats[1]+'-'+stats[5]+'\" target=\"_blank\">'+file+'</a><input type="hidden" name="flupload[]" value="'+stats[1]+'-'+stats[5]+'"><input type="hidden" name="fluploadtype[]" value="'+stats[2]+'"></td><td valign="top"> <a href="#uploafile" id="'+stats[1]+'-'+stats[5]+'" class="deleteimage">&nbsp;&nbsp;&nbsp;<img border="0" src="./img/plugins/ajaxupload/trash.png"></a></td>').attr("id",stats[1]);
		...
		else if ($_GET["cpmodul"] === "berita" || $_GET["cpmodul"] === "agenda")
		{
		new AjaxUpload(btnUpload, {
			action: '?ajax='+$_GET["cpmodul"]+'&aksi=uploadfile',
			name: 'uploadfile',
			...
			if (stats[2] !== "image/png" && stats[2] !== "image/jpeg")
				{ 
			$.ajax({
        type: 'POST',
        data: {
           
            filenyo: stats[1]+'-'+stats[5],
           
        },
        url: '?ajax='+$_GET["cpmodul"]+'&aksi=hapusuploadfiletmp'
    })
                    // extension is not allowed 
					status.text('Only JPG or PNG files are allowed');
					return false;
			...
			else
			{
				...
				{
				$('.berhasil').html('<img src="tmp/'+$_GET["cpmodul"]+'/'+stats[1]+'-'+stats[5]+'" alt="" />');
				$('#uploadhidden').val(stats[1]+'-'+stats[5]);
				$.ajax({
        type: 'POST',
        data: {
           
            filenyo: localStorage.getItem("filenyo"),
        ...
        url: '?ajax='+$_GET["cpmodul"]+'&aksi=hapusuploadfiletmp'
				...
				$('<li></li>').appendTo('#filelist').html('<img src="tmp/'+$_GET["cpmodul"]+'/'+stats[1]+'-'+stats[5]+'" alt="" />').addClass('berhasil');
				$('#uploadhidden').val(stats[1]+'-'+stats[5]);
				
}
////$("#upload").removeAttr("data-rule-required");	
////$("#upload").attr("data-rule-required","false");	
//$(".help-block.error").attr("for","upload").remove();
////$("span[for=upload].help-block.error").remove();
localStorage.setItem("filenyo", stats[1]+'-'+stats[5]);
		...
		else if ($_GET["cpmodul"] === "pengelola" || $_GET["cpmodul"] === "testimoni")
		{
		new AjaxUpload(btnUpload, {
			action: '?ajax='+$_GET["cpmodul"]+'&aksi=uploadfile',
			name: 'uploadfile',
...
localStorage.setItem("filenyo", stats[1]+'-'+stats[5]);
		...
		var btnUpload_pd=$('#upload_pd');
		var status_pd=$('#status_pd');

		new AjaxUpload(btnUpload_pd, {
			action: '?ajax='+$_GET["cpmodul"]+'&aksi=uploadfile',
			name: 'uploadfile',
			accept: '.jpg,.jpeg,.png',
			onSubmit: function(file, ext){
				 if (! (ext && /^(jpg|jpeg|png)$/.test(ext))){ 
                    // extension is not allowed 
					status_pd.text('Only JPG or PNG files are allowed');
					return false;
				}
				status_pd.html('<img src="./img/plugins/ajaxupload/uploading.gif"> Uploading...');
			},
			onComplete: function(file, response_pd){
				
				//On completion clear the status
				
				var stats_pd = response_pd.split("|");
				
			if (stats_pd[2] !== "image/png" && stats_pd[2] !== "image/jpeg")
				{ 
			$.ajax({
        type: 'POST',
        data: {
           
            filenyo_pd: stats_pd[1]+'-'+stats_pd[5],
        ...
        url: '?ajax='+$_GET["cpmodul"]+'&aksi=hapusuploadfiletmp'
			...
			else
			{
				...
				status_pd.text('');
				//Add uploaded file to list
				if(stats_pd[0]==="success"){
				
				if($('.berhasil_pd').length == '1')
				{
				$('.berhasil_pd').html('<img src="tmp/'+$_GET["cpmodul"]+'/'+stats_pd[1]+'-'+stats_pd[5]+'" alt="" />');
				$('#uploadhidden_pd').val(stats_pd[1]+'-'+stats_pd[5]);
				$.ajax({
        type: 'POST',
        data: {
           
            filenyo_pd: localStorage.getItem("filenyo_pd"),
        ...
        url: '?ajax='+$_GET["cpmodul"]+'&aksi=hapusuploadfiletmp'
				...
				$('<li></li>').appendTo('#filelist_pd').html('<img src="tmp/'+$_GET["cpmodul"]+'/'+stats_pd[1]+'-'+stats_pd[5]+'" alt="" />').addClass('berhasil_pd');
				$('#uploadhidden_pd').val(stats_pd[1]+'-'+stats_pd[5]);
...
//$(".help-block.error").attr("for","upload").remove();
////$("span[for=upload].help-block.error").remove();
localStorage.setItem("filenyo_pd", stats_pd[1]+'-'+stats_pd[5]);

	} else{
					status_pd.text('error!!!');
				}
		...
		$('#filelist').on('click','.deleteimage', function() {
			if(confirm('Hapus file ini ?'))
			{
   var idnyo = $(this).attr('id');
   var res = idnyo.split("-");
					//var namofile = res[0]+'-'+res[1];
...
$('#filelist').on('click','.deleteimagelamo', function() {
	if(confirm('Hapus file ini ?'))
	{
	var idnyo = $(this).attr('id');
    ...
    $.ajax({
        ...
        url: '?ajax='+$_GET["cpmodul"]+'&aksi=hapusfile',
         success: function() {
		 $("#file_"+idnyo).remove();
		
            //alert(idnyo+' berhasil dihapus '+namofile);
	...
	$("#formentripendaftaran input[name='status']").click(function(){
			var val = $(this).val();
			if(val=='1'){
				$('.pesanterima').show();
				$('.pesantolak').hide();
				...
				$('#message').prop('data-rule-required', true);
				$('#message').prop('data-rule-maxlength','160');
				
				$('#template2').removeAttr('required');
				$('#message2').removeAttr('data-rule-required');
				$('#message2').removeAttr('data-rule-maxlength');
				$('#rm').removeAttr('data-rule-required');
				...
				$('#template2').val("");
				$('#message2').val("");
				$('#template2').prop('required', true);
				$('#message2').prop('data-rule-required', true);
				$('#message2').prop('data-rule-maxlength','160');
				
				$('#template').removeAttr('required');
				$('#message').removeAttr('data-rule-required');
				$('#message').removeAttr('data-rule-maxlength');
				$('#rm').removeAttr('data-rule-required');
				...
				$('#rm').prop('data-rule-required', true);
        ...
        $('#message2').val($(this).val()); 
    });	
	});
	
function cekgalonyo() {
  var checkname = document.getElementsByName('pilih[]');
  for (i = 0; i < checkname.length; i++)
  checkname[i].checked = document.formentri.allbox.checked? true:false
}

function cekcheckbox(conf) {
        var opt = document.getElementsByName('pilih[]');
        ada = false;
